...
Gate 2 — **Cross-environment replay.** Byte-for-byte replay has passed
on short chains in one process on one machine. The stated bar — "run
twice on different machines" — has not been attempted. Node versions,
platforms, and long chains (thousands of epochs) must agree before the
substrate is called stable.
